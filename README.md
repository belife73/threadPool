...
1. **任务队列 (Task Queue)**
   ...
   - **并发控制**：
     ...
     - 线程的阻塞（当队列为空时）使用 **Mutex + Condition Variable**，避免空转浪费 CP U。
...
1. **初始化**: `thrdpool_create` -> 创建 TaskQueue -> 创建 Worker Threads。
2. **生产任务**: `thrdpool_post` -> 封装 Task -> Spinlock 加锁 -> 入队 -> Spinlock 解锁 -> `pthread_cond_signal` 唤醒工作线程。
...
在 `thrd_pool.h` 中定义了对外接口：

| **函数原型**                                                 | **说明**                   | **参数与返回值**                                             |
| ------------------------------------------------------------ | -------------------------- | ------------------------------------------------------------ |
| `thrdpool_t *thrdpool_create(int thrd_count)`                | 创建并初始化线程池         | **thrd_count**: 线程数量 **返回**: 线程池句柄，失败返回 NULL |
| `int thrdpool_post(thrdpool_t *pool, handler_pt func, void *arg)` | 向线程池投递任务           | **pool**: 句柄 **func**: 任务回调函数 **arg**: 上下文参数 **返回**: 0 成功, -1 失败 |
| `void thrdpool_terminate(thrdpool_t * pool)`                 | 通知线程池停止（非阻塞）   | **pool**: 句柄 设置退出标志，唤醒所有阻塞线程，不再接收新任务 |
| `void thrdpool_waitdone(thrdpool_t *pool)`                   | 等待所有线程结束并销毁资源 | **pool**: 句柄 阻塞直到所有工作线程退出，释放队列和池的内存  |

------

## 4. 构建与编译指南

项目包含 C 和 C++ 代码，通常编译为动态库供业务链接。

### 4.1 编译核心库 (`libthrd_pool.so`)

```Bash
# 1. 编译目标文件 (Position Independent Code)
gcc thrd_pool.c -c -fPIC -o thrd_pool.o
...
- **测试逻辑**：
  ...
  4. 当 `done` 达到 1000 时，调用 `thrdpool_terminate`。
  5. 主线程调用 `thrdpool_waitdone` 回收资源。

- **编译与运行**：

  ```Bash
  gcc main.c -o main -I./ -L./ -lthrdpool -lpthread
  export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:.
  ./main
  ```

### 5.2 单元测试 (`taskqueue_test.cc`)
...
- **结果解读示例**：
  ...
  ```
  <结束时间戳> <开始时间戳> used:<毫秒数> exec per sec:<每秒执行任务数>
  ```
...
2. **如何避免虚假唤醒？**
   ...
   - 在 `thrd_pool.c` 的 `__get_task` 函数中，使用了 `while` 循环检查队列状态：
...
3. **内存泄漏检测**
   ...
   - 建议使用 Valgrind 运行 `main` 或 `taskqueue_test`：

     ```Bash
     valgrind --leak-check=full ./main
     ```

   - `thrdpool_waitdone` 函数中包含了对剩余任务和线程资源的清理逻辑，确保正常退出时无泄漏。